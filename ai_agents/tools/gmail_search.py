...
def find_email_by_subject(service, subject, include_raw_data=False):
    """
    Find the first email matching a subject line and save it to a JSON file.
    ...
    Args:
        ...
        include_raw_data (bool, optional): Whether to include raw base64 data in JSON. 
                                         Defaults to False.
        
    Returns:
        dict or None: Dictionary containing:
            - message: Full Gmail message object
            - email_text: Decoded body text
    ...
    Note:
        ...
        - Automatically creates output_files directory if it doesn't exist
    """
    results = service.users().messages().list(userId="me", q=f'subject:"{subject}"').execute()
    messages = results.get("messages", [])

    if messages:
        message_id = messages[0]["id"]  # Get first match
...
def search_emails(service, query, max_results=10):
    ...
    for msg in messages:
        try:
            ...
            body_preview = body_text[:30] if body_text else ""
